...
export class Coordinate {
  ...
  directionTo(otherCoordinate: Coordinate): Direction {
    ...
    if (dx === 0 && dy === -1) {
      ...
      throw new Error(
        `The coordinate is too far away!, Difference is (${dx}, ${dy})`,
      );
    }
  }
}
...
export class Snake {
  ...
  static fromSnakeInfo(snakeInfo: SnakeInfo, mapWidth: number, map: GameMap) {
    ...
    const coordinates = positions.map((position) =>
      Coordinate.fromPosition(position, mapWidth),
    );
...
export class GameMap {
  ...
  isTileFree(coordinate: Coordinate) {
    return (
      this.getTileType(coordinate) === TileType.Empty ||
      this.getTileType(coordinate) === TileType.Food
    );
  }
}
