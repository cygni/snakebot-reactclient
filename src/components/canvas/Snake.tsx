...
function Snake({ snake }: Props) {
  ...
  function drawLine(line: TilePosition[]) {
    ...
    const isHorizontal: boolean = line[0].y === line[1].y;
  ...
  function renderSnakeBody() {
    ...
    for (let i = 1; i < snake.positions.length; i++) {
      ...
      if (currentMovementHorizontal !== lastMovementHorizontal) {
        // new line
    ...
    return lines.map((line, lineIndex) => {
      ...
      return (
        <Group key={lineIndex}>
          {drawLine(line)}
          {drawRoundedPart(lastTile, lines, lineIndex)}
        </Group>
      );
    });
  }

  function drawRoundedPart(tile: TilePosition, lines: TilePosition[][], lineIndex: number) {
